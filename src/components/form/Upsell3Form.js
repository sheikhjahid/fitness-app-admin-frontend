...
import Upsell3ProductsJson from '../../../../helpers/json/master.json';
...
const Upsell3Products = [
  { value: 43, label: '3x Months supply Box' },
  { value: 44, label: '2x Months supply Box' },
  { value: 45, label: '1x Months supply Box' },
];
const StatusTimer = [
  { value: 'true', label: 'On' },
  { value: 'false', label: 'Off' },
...
export default function Upsell3Form({ validateUpsell3Form }) {
  ...
  useEffect(() => {
    validateUpsell3Form.current = validateCurrentForm;
    autofillData();
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < Upsell3ProductsJson.upsells.length; i += 1) {
      if (Upsell3ProductsJson.upsells[i].url === urlVal) {
        const arr = Upsell3ProductsJson.upsells[i].products;
        setProductsArr(arr);
  ...
  const autofillData = () => {
    ...
    if (arr.length >= 3) {
      const obj1 = arr[2];
      const settings = arr[2].settings;
      const items = arr[2].items;
        ...
        <Grid container spacing={2}>
          ...
          <Grid item md={6}>
            <FormDropdown
              ...
              message="Required"
              options={Upsell3ProductsJson.urls}
              require
            />
          </Grid>
          <Grid item md={6}>
...
Upsell3Form.propTypes = {
  validateUpsell3Form: PropTypes.any,
};
