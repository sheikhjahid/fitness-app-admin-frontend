...
import Upsell4ProductsJson from '../../../../helpers/json/master.json';
...
const Upsell4Products = [
  { value: 370, label: '3 box' },
  { value: 371, label: '2 box' },
  { value: 372, label: '1 box' },
];

export default function Upsell4Form({ validateUpsell4Form }) {
  ...
  useEffect(() => {
    validateUpsell4Form.current = validateCurrentForm;
    autofillData();
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < Upsell4ProductsJson.upsells.length; i += 1) {
      if (Upsell4ProductsJson.upsells[i].url === urlVal) {
        const arr = Upsell4ProductsJson.upsells[i].products;
        setProductsArr(arr);
  ...
  const autofillData = () => {
    ...
    if (arr.length >= 4) {
      const obj1 = arr[3];
      const settings = arr[3].settings;
      const items = arr[3].items;
        ...
        <Grid container spacing={2}>
          ...
          <Grid item md={6}>
            <FormDropdown
              ...
              message="Required"
              options={Upsell4ProductsJson.urls}
              require
            />
          </Grid>
          <Grid item md={6}>
...
Upsell4Form.propTypes = {
  validateUpsell4Form: PropTypes.any,
};
